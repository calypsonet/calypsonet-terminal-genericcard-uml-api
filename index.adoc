...
[[op_GenericCardApiFactory_createGenericCardTransactionManager]]
===== Create Generic Card Transaction Manager

[cols="^.^1h,.^5", grid=rows, frame=all]
|===
| Signature
| `createGenericCardTransactionManager(` +
`{nbsp}{nbsp}{nbsp}{nbsp}reader: CardReader,` +
`{nbsp}{nbsp}{nbsp}{nbsp}card: SmartCard` +
`) -> GenericCardTransactionManager`
| Since          | 1.0
| Description    | Returns a new, freshly initialised <<type_GenericCardTransactionManager,`GenericCardTransactionManager`>>, bound to the supplied reader and to the initial card data provided by the selection process.
| Parameters
...
| Returns        | A <<type_GenericCardTransactionManager,`GenericCardTransactionManager`>>.
...
[cols="^.^1h,.^5", grid=rows, frame=all]
|===
| Signature      | `addSuccessfulStatusWord(statusWord: Int) -> Self`
| Since          | 1.0
| Description
...
| Parameters     | `statusWord` -- the status word to add.
| Returns        | The current instance (`Self`).
| Pre-conditions | *Argument* -- every parameter is non-`null`. +
  *Argument* -- `statusWord` is in the range `[0x0000, 0xFFFF]`.
| Errors         | None.
|===

[[type_GenericCardTransactionManager]]
==== Generic Card Transaction Manager

[cols="^.^1h,.^5", grid=rows, frame=all]
|===
| Name       | `GenericCardTransactionManager`
| Kind       | Interface
| Namespace  | `genericcard`
...
| Provides the basic operations required to prepare, identify and process APDU exchanges with an ISO/IEC 7816-4
  card. Extending `IsoCardTransactionManager` grants access to the multi-channel cast
  `asMultichannelCardTransactionManager()` defined by <<ref_cna_tr,CNA-TR-API>> when the underlying card supports
  multi-channel. An instance is obtained via <<op_GenericCardApiFactory_createGenericCardTransactionManager,`GenericCardApiFactory.createGenericCardTransactionManager`>>.
|===

The `GenericCardTransactionManager` uses a *prepare/process* model inherited from <<ref_cna_tr,CNA-TR-API>>: the application prepares
APDU commands with `prepareCommand`, `prepareCommandWithId` or `prepareCommandWithMaxDuration` and processes them through the inherited `processCommands()`
operation. Each prepared command MAY carry an optional *command identifier* (`commandId`) supplied by the
application, used to retrieve the corresponding response or to identify the failing command in case of error.
Each prepared command MAY also carry an optional *maximum duration* (`maxDuration`, in microseconds, the effective
resolution of the measurement depending on the implementation) used to
control the APDU exchange execution time at the individual-command level -- for example to detect a relay
attack.

[[op_GenericCardTransactionManager_getLastExecutionResponse]]
===== Get Last Execution Response

[cols="^.^1h,.^5", grid=rows, frame=all]
|===
| Signature      | `getLastExecutionResponse(commandId: Int) -> ByteArray?`
| Since          | 2.0
| Description
| Returns the response of the command identified by `commandId`. If several prepared commands share the same
...
| Parameters     | `commandId` -- the application-supplied identifier of the command whose response is requested.
| Returns        | The non-empty response bytes, or `null` if no command with the supplied identifier produced a response.
| Pre-conditions | *Argument* -- every parameter is non-`null`.
| Errors         | None.
|===

[[op_GenericCardTransactionManager_getLastExecutionResponses]]
...
[[op_GenericCardTransactionManager_prepareCommand]]
===== Prepare Command

[cols="^.^1h,.^5", grid=rows, frame=all]
|===
| Signature      | `prepareCommand(apdu: ByteArray) -> Self`
| Since          | 2.0
...
[[op_GenericCardTransactionManager_prepareCommandWithId]]
===== Prepare Command With ID

[cols="^.^1h,.^5", grid=rows, frame=all]
|===
| Signature
| `prepareCommandWithId(` +
...
| Prepares an APDU command with an application-supplied identifier. The identifier MAY later be used to retrieve
  the corresponding response via <<op_GenericCardTransactionManager_getLastExecutionResponse,`getLastExecutionResponse`>>.
| Parameters
...
[[op_GenericCardTransactionManager_prepareCommandWithMaxDuration]]
===== Prepare Command With Max Duration

[cols="^.^1h,.^5", grid=rows, frame=all]
|===
| Signature
| `prepareCommandWithMaxDuration(` +
`{nbsp}{nbsp}{nbsp}{nbsp}commandId: Int,` +
`{nbsp}{nbsp}{nbsp}{nbsp}apdu: ByteArray,` +
...
| `commandId`   -- the application-supplied identifier of this command. +
  `apdu`        -- a non-empty `ByteArray` containing the raw APDU command. +
  `maxDuration` -- the maximum tolerated duration of the APDU exchange, in microseconds.
| Returns        | The current instance (`Self`).
| Pre-conditions | *Argument* -- every parameter is non-`null`. +
